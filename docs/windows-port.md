...
Citations are `file:line` against `integration/0.13.0` at `8103e70`.
...
| `agent_parley/checkpoints.py:7`, `:542-563` | `event_lock()`: shared (`LOCK_SH`) for readers and append writers, exclusive for maintenance, blocking or bounded. |
...
| Site | Use |
| --- | --- |
| `agent_parley/cli.py:2922` | Detached service start. |
| `agent_parley/hook.py:374` | Hook relaunches a dead service. |
| `agent_parley/supervision.py:5049` | Supervisor starts a resume launcher. |
...
| `agent_parley/cli.py:2898-2907` | Port probe with `SO_REUSEADDR`. On Windows that option lets a second socket bind a port already in use, so the probe cannot detect an occupied port; `HTTPServer` also sets it through `allow_reuse_address`. |

### File modes, directory sync and shell assumptions

| Site | Use |
| --- | --- |
| `agent_parley/cli.py:2767-2770` | `Bridge.__init__` refuses a home whose mode has any group or other bit. On Windows `st_mode` of a directory reports `0o777`, so every home would be refused. |
...
| `agent_parley/recovery.py:69-70`, `agent_parley/store.py:339`, `agent_parley/roster.py:863`, `agent_parley/opencode.py:180`, `agent_parley/terminal.py:656` | `0o600` and `0o700` modes for private state. Windows ignores all bits except read-only; privacy would come from the profile directory ACL. |
...
`.github/workflows/check.yml:41-63`: the `wsl` job runs the suite inside
...
| Port probe | `SO_EXCLUSIVEADDRUSE` instead of `SO_REUSEADDR` on Windows, and `allow_reuse_address = False` on the service. | `Bridge` start in `cli.py`, `server.Server`. | S | High if missed: two services could bind one port. |
| Hook client | Ship the hook as a Python entry point (`python -m agent_parley.hook`) or a `.cmd` wrapper; keep the Bash client on POSIX for its start-up cost. | `hook.write_client()` and the hook registration in `cli.py`. | M | Medium: hook start-up latency on every native tool call; must be measured. |
