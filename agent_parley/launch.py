"""Runs one participant's native CLI with coordination wired into it."""
...
from agent_parley.copilot import COPILOT_EVENTS, configure_copilot
from agent_parley.lazy import DeferredCallable, deferred, deferred_module
from agent_parley.worktrees import Worktrees
...
if TYPE_CHECKING:
    ...
    dialogs = deferred("dialogs")
    gemini = deferred("gemini")
    opencode = deferred("opencode")
...
class Launch(Worktrees):
    """Registers a lane and starts its native client inside it."""
